...
class Main {
    public static void main(String[] args) throws IOException {
        ...
        out.println("before del");
        t2.printByLayer(outs);
        outs.writeBytes(System.lineSeparator());
        t2.preorderPrint(outs);
        LinkedList.Link list = t2.GraphNodes.head;
        while (list.graphNode.getKey()!=11)
            list=list.next;
        out.println("*****");
        out.println(list.graphNode.getKey());
        out.println("*****");
        t2.deleteNode(list.graphNode);
        out.println("after del");
        t2.printByLayer(outs);
        outs.writeBytes(System.lineSeparator());
        t2.preorderPrint(outs);
        out.println("fin");
